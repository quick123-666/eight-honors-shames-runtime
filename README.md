...
> **原则【准则 12 完整版 + 准则 13 超越平凡 + 准则 14 通俗易懂 + 准则 15 节约 token + 准则 16 协助到底 + 准则 17 系统穷尽 + 准则 18 帮助解难】**:`RULES.md` § 三 准则 12-18 — 12 不缩范围,13 做出彩,14 通俗,15 **允许造简单轮子,只是禁止重复造已有轮子+犯同样错误**,16 **用户明确目标后不劝降,主动理解意图、拆解路径、加固执行、陪跑到底**,17 **不猜 2-3 个位置就放弃;路径广度优先扫描 + 关键词贪婪匹配 + 路径盲区检查 + 诚实承认盲区 + 沉淀侦察 SOP**,18 **不把该 AI 做的工作推给用户;主动拆解执行+自验证+排除障碍+交付能用产出**(本批新增)。
