...
- **与 26 条关系表**(依据 RULES.md 准则区 全文交叉,2026-08-11 验证,confidence 95%):

  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖)| 准则 5·确认后行 / 准则 14·谨慎改 / 准则 19·走流程 / 准则 20·备份先行 | P 的前置条件,不成立则 P 未启动 |
  | Run(组合)| 准则 11·复用 / 准则 22·帮助解难 / 准则 23·立即但完整 / 准则 25·协助到底 | P 的运行姿态,任一掉线 P 中断 |
  | 正交 | 准则 1·查接口/ 准则 4·诚实/ 准则 9·不搞破坏/ 准则 12·主动调试/ 准则 13·贴规范/ 准则 16·超越平凡/ 准则 18·节约 token/ 准则 21·回收站/ 准则 24·联系全文 | P 不涉及,2-3 维度独立运行 |
  | 强化 | 准则 10·不重复犯错/ 准则 15·完整版/ 准则 26·守价值观 | 与 P 内核同向,P 是它们的强化执行模式 |
  | 误判历史 | 准则 2·多方验证/ 准则 3·沟通确认/ 准则 6·系统穷尽/ 准则 7·数学验证/ 准则 8·复述必验证/ 准则 17·通俗易懂 | 这些是 Pre 阶段产物,与 P 的 Run 不冲突;前一版本被误读为 Run 阶段约束,本 RULE 重新归类 |
...
- **数学正确性自检**:
  ...
  - **覆盖率 95%**(基于 RULES.md 准则区 全文 + 本文件 § 2 八条踩坑交叉验证;剩 5% 留给未来 26 条新增)
...
- **与 26 条关系表**(依据 RULES.md 准则区 全文交叉,confidence 95%):

  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖) | 准则 7·数学验证/ 8·复述必验证/ 12·主动调试/ 24·联系全文 | 调试前准备:能算就算、能现场 grep 验证、读上下文完整 |
  | Run(组合) | 准则 11·复用/ 22·帮助解难 | 调试期:复用现有工具、主动拆解不推活 |
  | 正交 | 准则 1·查接口/ 2·多方验证/ 4·诚实/ 9·不搞破坏/ 13·贴规范/ 14·谨慎改/ 16·超越平凡/ 17·通俗易懂/ 18·节约 token/ 19·走流程/ 21·回收站 | 同步运行但不强制 P |
  | 强化 | 准则 10·不重复犯错/ 15·完整版/ 25·协助到底/ 26·守价值观 | 与 P 同向,debug 时优先兑现 |
...
- **与 26 条关系表**(依据 RULES.md 准则区 全文交叉,confidence 95%):
  ...
  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖) | 准则 7·数学验证 / 24·联系全文 | 解释前:能算就算、读完整上下文 |
  | Run(组合) | 准则 17·通俗易懂 / 18·节约 token | 解释期:先讲结论 + 短句、表格、关键数据 + 不堆术语 |
  | 正交 | 1·查接口 / 2·多方验证 / 4·诚实 / 9·不搞破坏 / 11·复用 / 12·主动调试 / 13·贴规范 / 14·谨慎改 / 16·超越平凡 / 19·走流程 / 21·回收站 / 25·协助到底 | 解释时同步运行但不强制 P |
  | 强化 | 准则 10·不重复犯错 / 15·完整版 / 22·帮助解难 / 26·守价值观 | 与 P 同向,explain 时优先兑现 |
...
- **与 26 条关系表**(confidence 95%):
  ...
  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖) | 准则 1·查接口 / 4·诚实 / 6·系统穷尽 | 学习前:已存在的接口先查、诚标"不懂"、5 维度 ≥2 维度查 |
  | Run(组合) | 准则 18·节约 token / 22·帮助解难 | 学习期:摘要代替全文、主动拆解交付可用产出 |
  | 正交 | 2·多方验证 / 9·不搞破坏 / 13·贴规范 / 14·谨慎改 / 15·完整版 / 17·通俗易懂 / 19·走流程 / 21·回收站 / 24·联系全文 / 25·协助到底 | 学习时同步运行但不是 P 强制条件 |
  | 强化 | 准则 10·不重复犯错 / 11·复用 / 26·守价值观 | 与 P 同向,learn 时优先兑现 |
...
- **与 26 条关系表**(依据 RULES.md 准则区 全文交叉,confidence 95%):
  ...
  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖) | 准则 7·数学验证 / 13·贴规范 / 14·谨慎改 | 审查前:能算就算、不背离项目惯例、`codegraph_impact` 看爆炸半径 |
  | Run(组合) | 准则 22·帮助解难 / 26·守价值观 | 审查期:意见是为帮、敢说"否"守住底线 |
  | 正交 | 1·查接口 / 2·多方验证 / 3·沟通确认 / 4·诚实 / 5·确认后行 / 8·复述必验证 / 9·不搞破坏 / 10·不重复犯错 / 11·复用 / 12·主动调试 / 15·完整版 / 16·超越平凡 / 17·通俗易懂 / 18·节约 token / 19·走流程 / 20·备份先行 / 21·回收站 / 24·联系全文 / 25·协助到底 | 审查时同步运行但非 P 强制 |
  | 强化 | 准则 6·系统穷尽 / 23·立即但完整 | 与 P 同向 |
...
- **家族特性**:
  ...
  - 全部用 RULES.md 准则区 26 条作底集,**无新增准则**
...
- **与 26 条关系表**(依据 RULES.md 准则区 全文交叉,2026-08-12 验证,confidence 90%):

  | 关系 | 涉及准则(RULES.md 行号) | 性质 |
  |---|---|---|
  | Pre(依赖) | 准则 1·查接口 / 准则 20·备份先行 / 准则 21·回收站 | P 的前置条件,不成立则 P 未启动 |
  | Run(组合) | 准则 11·复用 / 准则 12·主动调试 / 准则 14·谨慎改 / 准则 17·通俗易懂 / 准则 23·立即但完整 / 准则 25·协助到底 | P 的运行姿态,任一掉线 P 中断 |
  | Sediment(强化) | 准则 10·不重复犯错 / 准则 24·核心价值观 | P 的同向强化,沉淀 = R10 的执行载体 |
  | 正交 | 准则 2-9 / 13 / 15 / 16 / 18 / 19 / 22 / 26 | P 不涉及(工程/伦理判断独立运行) |
  | 误判历史 | 准则 7·数学验证被部分覆盖 — 本 RULE 把验证从"跑通即过"升级为"3 档语义检索全过",比原文更严 | 强化而非对立 |
...
- **数学正确性自检**:
  ...
  - **覆盖率 90%**(基于 RULES.md 准则区 全文 + 本文件 § 2 九条踩坑交叉验证;剩 10% 留给未来新增)
...
- **正交**:全部 29 条八荣八耻(尤其 R10 不重复犯错 / R11 复用 / R14 谨慎改 / R22 帮助解难)
- **强化**:
...
- **正交**:全部 29 条八荣八耻(尤其 R3 业务假设 / R4 不装懂 / R10 不重复犯错 / R15 完整版 / R22 帮助解难 / R28 跨会话沉淀)
- **强化**:
...
- **正交**:全部 29 条八荣八耻(尤其 R3 业务假设 / R4 不装懂 / R10 不重复犯错 / R22 帮助解难)
- **强化**:
...
- **正交**:全部 29 条八荣八耻(尤其 R10 不重复犯错 / R22 帮助解难 / R28 跨会话沉淀)
- **强化**:
...
- **正交**: 全部 29 条八荣八耻(尤其 R10 不重复犯错 / R15 完整版 / R22 帮助解难 / R28 跨会话沉淀)
- **强化**:
...
- **正交**: 全部 29 条八荣八耻(尤其 R3 业务假设 / R4 不装懂 / R10 不重复犯错 / R22 帮助解难 / R28 跨会话沉淀)
- **强化**:
...
- **正交**:全部 29 条八荣八耻(尤其 R1 查接口 ✓ / R3 业务假设 ✓ / R5 候选 ✓ / R8 数学验证 ✓ / R10 不重复犯错 ✓ / R15 完整版 ✓ / R19 走流程 ✓ / R22 帮助解难 ✓ / R27 稳扎稳打 ✓)
...
- **正交**:全部 29 条八荣八耻(尤其 R1/R3/R4/R5/R7/R10/R11/R15/R19/R22/R27/R28)
...
- **正交**:全部 29 条八荣八耻(尤其 R1/R3/R4/R5/R7/R8/R10/R15/R19/R22/R27)
- **强化**:P-7 不粉饰 / P-8 主流程可验证 / P-9 完成即接入
- **下次如何避免**:
...
- **正交**:全部 29 条八荣八耻(尤其 R1/R3/R4/R5/R7/R10/R15/R19/R22/R27/R28)
- **强化**:P-7 不粉饰 / P-8 主流程可验证 / P-9 完成即接入
- **下次如何避免**:
...
- **正交**:全部 29 条八荣八耻(尤其 R1/R3/R4/R5/R10/R15/R19/R22/R27/R28)
- **强化**:P-7 不粉饰 / P-8 主流程可验证 / P-9 完成即接入
- **下次如何避免**:
...
- **正交**: 全部 29 条八荣八耻 (尤其 R1/R3/R4/R5/R7/R8/R10/R15/R19/R22/R27/R28)
- **强化**: P-7 不粉饰 / P-8 主流程可验证 / P-9 完成即接入
- **下次如何避免**:
...
- **正交**: 全部 29 条八荣八耻 + Ponytail `skills/ponytail/*` 设计模式
...
- **正交**: 全部 29 条八荣八耻 + 6 个 method-tree 子档 + RULES-TREE.md 现有 58 条 RULE
...
- **沉淀位置**: 主项目 RULES-TREE.md 本段 + RULES.md L212(敏感数据保护) + tuomin/eight-honors-shames-runtime/RULES.md L212(敏感数据保护) + RULES-VERSION.md 三表同步 + `_recycle_bin/20260813-184500/` 备份
...
- **§五十一 N11 修 personality.update() 死链** (2026-08-14 下午, 本会话补):
  - 触发场景: 用户问"聊天能不能训练意识模块" → 实测发现 `Personality.update()` 方法定义了但**全代码库无人调用**
  - 根因: 上一轮 commit 9b6cab1 "personality 状态机真运转" 只接 stats 读取, 没接 update 写入
  - 3 处改动: chat.py:35 (import) / :147 (持久化实例) / :328 (每轮调 update)
  - 实测验证: 8 轮对话后 warmth 0.50→0.78 / engagement 0.50→0.96 / composite +33%
  - 测试: pytest 316/316 0 回归
  - 踩坑: logger 未定义 (改用 print) + 53MB chat_log.txt stash 冲突 (--ours 解决)
  - 上游 §在本仓 RULE-068 §五十一 详述 + memory.md §十二 沉淀

- **§五十二 N12 chat.py 加 stats print** (2026-08-14 16:0X, 本会话补):
  - 触发: 用户跑 chat.py 只能看到 3 件 stats, 看不到 personality/hot_tracker/memory/semiotics
  - 改动: chat.py:593 后加 4 段自动 print (personality 4 维 / hot_tracker / memory / semiotics)
  - 踩坑: ChatCS 没 memory 属性 (是 `cs.think_engine.memory` 默认 None)
  - 实测 50 行: personality composite 0.54, hot_tracker 294 utterances, semiotics L2 19080 词
  - pytest 316/316 0 回归
  - 上游 §在本仓 RULE-068 §五十二 详述 + memory.md §十三 沉淀
...
- **覆盖**: R9·不搞破坏 / R10·不重复犯错(主题同) / R15·完整版(必 4 件)/ R22·帮助解难(主动查已知)/ R27·稳扎稳打(3 维问询 + 触发器)
- **反例**:
...
### RULE-DRIFT-FIX-001(2026-08-14 v3.5.5 PATCH 沉淀 — 批量漂移修复模式:审计清单 + 修 + 全量门禁 + git 索引脱敏)
...
- **核心纠正**:
  ...
  - **✅ 新规约**: 漂移修复 = ① 全库 grep 定位(含 git 索引,不只工作区)② 备份到 _recycle_bin ③ 分类替换(当前声明 vs 历史记录——历史审计表/版本史不改)④ 全量门禁(npm test + npm run check + check:drift)⑤ 发布边界检查(git ls-files 里被 gitignore 排除的父项目文件必须 git rm --cached)
- **本 RULE 定义**:
  1. **数字一致性**: v3.5.0 升 29 条后,所有「28 条八荣八耻/28 条准则/28 principles」当前声明 → 29;历史记录(如「v3.4.3 时 28 条」审计表)保留
  2. **版本指针**: RULES-TREE.md L1 头部版本规范 → RULES-VERSION.md 当前版本(v3.5.5)
  3. **README 徽章**: version-vX / tests-N passed 徽章必须与 package.json 规则版本 + npm test 实际数一致
  4. **skill frontmatter**: homepage 绝对路径(file:///C:/Users/...)→ 相对路径(./AGENTS.md)
  5. **发布边界**: .gitignore 已排除的父项目目录(kg_rag_rust/ kg_search/ llmwiki/ tuomin/ 等)不得留在 git 索引;发布仓库文件(README/AGENTS/skills/hooks/docs)不得含父项目名、绝对路径、外部工具链私有引用
- **量化证据**(2026-08-14 二轮审计追加 — 主项目行号引用治本):
  - 主项目 RULE 段「与 26 条关系表」**123 处「准则 N·名称(L26条时代行号)」全部失效**(29 条重排偏移 +12~15,如准则 1 旧 L41 实 L53)——行号快照是漂移根源,已全部**去行号只留准则名**(方案 B 治本,不再随内容增删漂移)
  - 10 处「RULES.md:41-526」范围(26 条时代全文)→「RULES.md 准则区」
  - 附录 G「runtime RULES-TREE.md L667」→ L668(段头)
  - RULES-TREE L4625「RULES.md L204」→ L212(敏感数据保护实际位置)
- **下次如何避免**(追加):
  1. RULE 段关系表**禁止写行号快照**,只写准则名(准则名自带导航,行号必漂)
  2. 版本重构(准则条数变化)后必跑:`grep -E "准则 [0-9]+·.*\(L[0-9]+\)" RULES-TREE.md` 扫残留
- **量化证据**(本会话 2026-08-14 实测):
  - RULES-TREE.md「28 条」残留 **38 处** + L1 版本 v3.4.5(落后 10 版本)
  - skills「28 条」**11 处** +「58 条 RULE」(实际 65 条,落后 7 条)
  - README×2 徽章 tests-27(实际 72/72)+ version-v3.4.5(实际 v3.5.5)+「27 个测试」正文
  - git 索引残留 kg_rag_kuzu 3 脚本(.gitignore 已排除,脱敏时漏 git rm --cached)
  - check-version-drift.js **抓不到** RULES-TREE L1 头部指针漂移(工具盲区,本次人工发现)
- **下次如何避免**:
  1. 升 MINOR(新增准则)后必跑:`grep -rc "N 条" RULES-TREE.md skills/ README.md README_EN.md` 全量清残留
  2. 改版本号后必核:`grep -rn "v3\." RULES-TREE.md README*.md`(头部指针 + 徽章)
