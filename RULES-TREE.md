...
- **本 RULE 定义**(8 条降级映射):
  ...
  | **R3 · 妄想业务** | 主动列假设 + 信心度 | `SemanticaBridge.record_decision(category, scenario, reasoning)` |
  ...
  | **R11 · 复用** | 主动扫项目函数 | `Core.to_dict()` / `SemanticaBridge.query_decisions()` |
  | **R19 · 走流程** | 备份 → 预览 → 确认 → 执行 → 验证 → 沉淀 | 沉淀环节走 `decisions_log.json` + `_persist_decision` |
  | **R28 · 跨会话沉淀** | 踩坑 / 架构决策 / 用户偏好必须落盘 | `SemanticaBridge.auto_record_decision()` + `load_history_to_semantica()` |
- **未替代的准则**(保留 21 条):
...
# RULE-30-MODULE-INTEGRATION-001 (2026-08-18 MINOR 增量 · v3.10.0 · 第三十条准则「全功能打通」沉淀)

> **下次谁交付涉及多模块 / 多接口 / 多依赖的功能必读本 RULE。** 加新模块 / 接口 / 配置 / 集成前必查。
> 状态: 生效 (2026-08-18 MINOR v3.10.0 新增第 30 条「全功能打通」)

- **触发**:
  - 写完一个模块 / 函数 / 接口,但从未被任何上游调用 → **孤儿节点 = 漏解**
  - 配置写了(env / config / .env.example)但代码未读取或读了但未生效 → **配置漂移 = 漏接**
  - 依赖装了(`package.json` / `requirements.txt`)但未在代码里 import / require → **依赖虚装 = 漏接**
  - 单测 100% 过但集成测 / 端到端测 0% 过 → **链路失守 = 漏跑**
  - 接口签名(参数类型 / 返回值 / 异常)上下游对不上,运行时才暴露 → **接口漂移 = 漏解**
  - 任何"我能跑啊"型声明,但跑通链路从未走过 1 次完整路径 → **不可信交付 = 漏跑**

- **核心纠正**:
  - **❌ 旧认知**: "代码写完 + 单测过 = 完成"
  - **✅ 新规约**: "代码写完 + 单测过 + 端到端链路跑通 + 真实调用方存在 = 完成"
  - **❌ 旧认知**: "主路径跑通 = 全功能打通"
  - **✅ 新规约**(v3.10.0 补):"全功能" = 主路径 + 错误路径 + 降级路径 + 边界场景 **全部走通 1 次**;只看主跑通 = 漏接 30% 场景 = 假打通

- **"全"的精确定义**(v3.10.0 补,用户原话「做了功能但不接通」对齐):
  - **主路径**(happy path):用户最常用流程必须跑通
  - **错误路径**:异常处理 / 4xx / 5xx 兜底必须跑通
  - **降级路径**:外部依赖失败时的兜底(重试 / 缓存 / 降级返回)必须跑通
  - **边界场景**:空值 / 极限值 / 并发 / 时序异常 必须跑通
  - **任一路径未走通 = 漏接** = R30 失守

- **本 RULE 定义 (4 件必查)**:
  1. **模块有真实调用方** — 每个新接口 / 函数 / 配置,至少有 1 个真实下游在用;**无孤儿节点**;新写接口无调用 = 视为漏解,**暂不做**
  ...
  4. **失败不藏** — 链路跑通时若发现某模块漏解 / 漏接,**先补**再标"完成",不假装没事;发现链路断点 → 主动暴露 + 写 RULES-TREE(标 RULE-30-XXX-NNN)

- **落地 4 步**(借鉴 RULE-FP-001 第一性原理复合算子):
  1. **列调用方清单** — 拿新模块问"谁会用?" → 写 1 行;无调用方 = **暂不做**(避免做孤儿)
  2. **跑端到端 1 次** — 输入最小数据 → 走完链路 → 看输出,记录在 commit message
  3. **过 4 问自检** — 依赖 / 配置 / 接口 / 降级,任一不达标 = **不算完成**,返工
  4. **失败必暴露** — 链路断点写 RULES-TREE(标 RULE-30-XXX-NNN),不当哑巴

- **量化证据**:
  - RULES.md 主项目 / 运行时副本 双副本同步加准则 30(全 ~50 行,含 4 件必查 + 协同 + 反例 + 落地 4 步 + 真实案例)
  - AGENTS.md 精简版同步加第 30 条(单行命令式)
  - RULES-VERSION.md 升 v3.10.0(MINOR)
  - 备份 `_backups/rules-add-30-<时间戳>/`(RULES.md 主 + 运行时副本 + AGENTS.md + RULES-VERSION.md 4 文件)

- **与既有准则的差异**:
  - vs **R15·完整版**: R15 重"范围不切片"(做 100%),R30 重"模块间接通"(做 100% **且通**)
  - vs **R16·超越平凡**: R16 重"代码质量补全",R30 重"**链路集成补全**"
  - vs **R12·验证**: R12 重"单点验证",R30 重"**端到端链路验证**"
  - vs **R28·跨会话沉淀**: R28 重"知识落盘",R30 重"**链路断点必落盘**"

- **真实案例**(沿用本项目历史):
  - **2026-08-17 v3.9.5 daemon spawn 修复**: daemon 写了但 `_ensureDaemon` 没探活 → 双启僵进程(57536/79776)根因 = **链路断**(daemon 启动后没人确认它真在跑);v3.9.5 加 `_ensureDaemon` + 端口轮询验证 = **R30 落地典型**
  - **2026-08-13 v3.4.4 双层 skill 架构**: skill 文档写了 7 份,但 hooks 注入未配 → 用户用不上 = **漏接**;v3.4.5 改为绑定 RULES-TREE 7 段范式 + `buildMethodTreeHint` 双层注入 = **R30 反例**

- **下次如何避免**:
  1. **新接口/函数/模块**: 写之前先答"谁会用",无调用方 = 不写
  2. **新配置项**: 写之后必跑"读取 → 应用 → 生效"链路 1 次
  3. **新依赖**: 装之后必在代码里 import/require,否则视同未装
  4. **集成测**: 关键路径 ≥ 80% 覆盖,不止单测
  5. **commit message**: 含"端到端跑通 1 次"声明(默认最低标准)
  6. **跨会话沉淀**: 漏解案例必落盘 RULES-TREE,标 RULE-30-XXX-NNN
...
- **关联**:
  - **RULE-FP-001** 第一性原理复合算子(R30 落地 4 步借鉴其分步模式)
  - **RULE-LOOP-002** commit 前对称检查(R30 的 commit message 自检是子集)
  - **RULE-DAEMON-STARTUP-FIX-001** daemon spawn 三件套(R30 真实案例)
  - **RULE-DECISIONS-LOG-SCHEMA-001** schema 约定(R30 沉淀 RULE-30-XXX-NNN 复用其命名)
...
- **本 RULE 定义**:
  ...
  4. **模式检测脚本**:`kg_rag_kuzu/scripts/check_mode.py` 实现 `detect_mode()` 函数,优先级 env > config > 默认
