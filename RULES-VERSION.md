...
| **v3.3.1** | **RULES.md 第五章 5.1-5.5"防空转循环机制"**(2026-08-13 新增)纳入版本号 + **全局 AGENTS.md 顶部版本号** v3.2.1→v3.3.0 漂移修复 + **多源终止信号**(全局"二点五"段 + F 档 `[COVER-ALL]` hook + 项目根"探针"段)未给优先级冲突显式化;沉淀 **RULE-LOOP-001**(死循环 = 三套结尾格式互不引用优先级 + 第五章未入版本号 + 死循环未沉淀 RULES-TREE) | 27 条(不变) | **当前最新** |
