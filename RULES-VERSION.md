...
| **v3.4.3** | **PATCH: 运行时副本三重漂移修复**(`tuomin/eight-honors-shames-runtime/RULES.md` 标题「二十六条」→「二十八条」 + 顶部 v3.3.0→v3.4.3 + 2.1/2.3 索引表 RULE-LOOP-001「❌ 待沉淀」→「✓ 已沉淀 L1175」 + 附录 E/F 24→28);**RULE-LOOP-002 对称检查 5→6 文件**;附录 E「20 字真言」验证闭环(20 个词条);RULE-LOOP-004 沉淀 | **28 条**(不变) | **当前最新** |
