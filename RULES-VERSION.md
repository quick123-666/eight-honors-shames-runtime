...
> **当前版本**:**v3.10.0**(2026-08-18,MINOR: 29 → 30 条 · 新增准则 30「全功能打通」— 与 R15 互补(R15 重范围不切片,R30 重模块间接通);端到端 4 件必查:模块有真实调用方 / 端到端链路测 / 集成清单自检 4 问 / 失败不藏;RULES-TREE 双副本沉淀 RULE-30-MODULE-INTEGRATION-001)
> **上一版本**:v3.5.4
> **关联文件**:RULES.md(完整版 **30 条**准) / AGENTS.md(精简版 **29 条**,两段式结构) / RULES-TREE.md(方法论沉淀)

## 最新变更摘要 v3.10.0 (2026-08-18)

- **本质**: MINOR 调优 — **29 → 30 条**;新增**准则 30 · 全功能打通**(以漏解模块为耻,以全功能打通为荣) — 与既有 R15 完整版互补(R15 重范围不切片,R30 重模块间接通);端到端 4 件必查:模块有真实调用方 / 端到端链路测 / 集成清单自检 4 问 / 失败不藏
- **不动结构**: 4 组不变,编号不变,30 条准则编号 1-30 不变
- **同步**:
  - `RULES.md` 新增准则 30 正文(L851 附近) + RULES-TREE 新增 `RULE-30-MODULE-INTEGRATION-001` 沉淀段
  - `build-adapters.js` 修复:extractPrinciples 不再截断至 21 条,改为全量镜像 AGENTS.md(当前 29 条);description/模板字符串更新
  - `tests/adapters.test.js` 断言从 21 条改为 ≥21 条全量检测
  - `README.md` / `README_EN.md` 「21 条」描述更新为「精简命令式清单(镜像 AGENTS.md,当前 29 条)」
  - `adapters/` 8 个适配文件(含 qclaw eight-honors SKILL.md)全量重建
  - 本文件(RULES-VERSION.md)头部版本号更新
- **不动**: `AGENTS.md` 精简清单 1-29 条不变(准则 30 仅在 RULES.md 完整版,精简版清单不在 AGENTS.md 正文);RULES-VERSION 顶部「不动结构:30 条准则(v3.10.0 起)」原为 v3.5.5 时代遗留,本次确认有效
- **回滚**: `cp _recycle_bin/20260818-122841-pre-v3.10.0-push/<file> <原路径>`(6 件备份);或本 commit 前 `git checkout <files>`
