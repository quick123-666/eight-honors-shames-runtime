...
- **判断标准**:
  ...
  - **任何用户输入(不管是不是 API 任务)**:**第一动作**都是搜项目内知识图谱 + 向量(本项目 `kg_rag_kuzu.semantic_search` / `kg_query.py` CLI,其他项目 `kg_rag_rust find "<q>" --top 5`),命中 P0/P1 RULE 立即走 RULE 流程 — 不只在文件系统找 — 治根图谱孤立节点问题(RULE-AUTO-GRAPH-LINK-001)
...
- **判断标准**:
  ...
  - **任何用户输入(强化)**:不管是不是侦察任务、是不是调试、是不是常规问答,**第一动作**都是搜知识图谱 + 向量 — `kg_rag_kuzu.semantic_search("<query>")`(本项目)/ `kg_rag_rust.exe find "<q>" --top 5`(其他项目),命中 P0 RULE(如 RULE-DSH-FIRST-RUN-001 / RULE-LOOP-001) → 强制走 RULE 流程,不省略不跳过。**违反 = R10 不重复犯错 + R28 跨会话沉淀失守**(参考反例 2026-08-18)
...
- **耻**:项目级知识(失败案例 / 架构决策 / 用户偏好 / 方法树)只在本会话上下文,会话结束即失
- **荣**:必落 `RULES-TREE.md` / wiki / `AGENTS.md`,跨会话可召回
- **逻辑**:单次会话结束上下文丢失 = 重复犯错 + 重复踩坑;沉淀 = 一次付出,永远受益
- **判断标准**:
  1. **踩坑必落盘**:失败案例修复后 24h 内,追加 `RULES-TREE.md` RULE-* 段(WHO/WHEN/DON'T/INSTEAD)
...
## 六、编码操作纪律(pi 侧执行细则 · 2026-08-12 新增)

> **触发**:任何写 / 改 / 重构 / 审查代码的任务(不只工具调用,还有代码本身质量)。**任务开始前必读本节;交付前必输出 6.6 guardrail 3 问**。
> 对应准则 12(验证)/ 14(谨慎改)/ 18(节约 token)。借鉴 super-code(工程派)+ Ponytail(减负派)实测:
> - super-code 优先级:正确性 → 清晰 → 必要健壮 → 简洁 → 微性能;**简洁永远不赢过正确性**。
> - Ponytail 实测:12 任务 -54% 行数且 100% 安全;但 OpenAI 弱模型 email 校验 79-98% slip(**砍安全会出事**)。
> - 核心调和:**准则 15"完整版" = 功能范围完整;本节 = 实现最小**。两者不矛盾——完整版不是代码膨胀的借口。
...
- 简洁永远不赢过正确性/清晰度:若压缩会砍掉"实际会发生的 case"的错误处理,或让 6 个月后的人读不懂 → **撤销该压缩,保留其余**
- 与准则 15 的关系:功能范围 = 完整版(不能缩);实现 = 最小(不能膨)

### 6.2 简化阶梯(写/改代码前过一遍)

> **机制(ladder 注意力杠杆)**:编号阶梯比散文段更有效——每个 rung 独立分块,attention 能在每个检查点上形成"是/否"离散决策,而不是整段扫过。所以**必须保持编号,不要改成段落**。

1. 这个抽象被用 ≥2 次了吗?单用 wrapper/helper → 内联
2. stdlib / 框架已有原语吗?→ 用它
3. 有没有死代码 / 未用 import / 死 flag?→ 删
4. 注释在复述代码吗?→ 删或改成"为什么"
5. 在防御"不可能发生的 case"吗?→ 删
6. 有没有未请求的日志 / 打印 / 配置文件?→ 删
7. 有没有占位 TODO / 空 catch?→ 补或删

### 6.3 反向守护清单(不可简化)

> **机制(反射反弹效应)**:指令"不要做 X"反而让模型注意力重写"X",更容易去做 X(counter-instruction backfire,pony 实测:堆校验规则反而产出更多坏校验器)。所以**只给类别、不给具体实例**——列出守护场景,不列出具体漏洞形态。改本条时禁止往清单里加具体反例(会反弹)。
...
- `X` = 被简化掉的具体物(**必须具名**)
- `Y` = 何时该加回(**必须可观察条件**,如 "3+ 处使用时")

### 6.5 生成过程纪律(准则 18 节约 token 的具体化)

- 定向 patch / 差异编辑,不整文件重写(除非:新文件 / <30 行 / 改动 >70% 行)
- 不生成未请求文件(测试 / README / config / CI);如觉得有价值,**一句话提议**,不直接生成
- 无 prose 前言("我要做 X 了")/ 后记("我改了 Y"),diff 自证
- 简化后**必跑测试 / 构建验证**(呼应准则 12)

### 6.6 自检 guardrail(交码前 3 问)

1. 我删掉了"实际会发生"的 case 处理吗?
2. 6 个月后的人类读得懂吗?
3. 我为了省行数牺牲正确性 / 安全了吗?

任一为是 → 撤销该处压缩,保留其余。

> 本板块沉淀为 RULE-CODING-001(见 RULES-TREE.md,对齐 RULE-FP-001 模式)。
...
| 准则 19 · 走流程 | 任何改动走"备份 → 预览 → 用户确认 → 执行 → 验证 → 沉淀"6 步 | *(本条是 R20 备份先行 + R5 确认后行 + R12 验证 + R15 完整版 在“改动闭环”场景的操作串联)* |
