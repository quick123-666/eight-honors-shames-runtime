// tests for buildMethodTreeHint() — 方法树硬话术核心(反漂移)
// 对标 eight-rules-hint.test.js ESM 写法(import 而非 require)
...
test("full 档返回激活 + NO DRIFT + 关闭方式", () => {
  ...
  assert.match(hint, /自问是否开方法树/, "full 档应引导 AI 主动自问");
});
...
test("ultra 档显示 ultra 档名 + 强制指令", () => {
  ...
  assert.match(hint, /方法树已激活/);
  assert.match(hint, /强制/, "ultra 档应强调强制每任务 mr run");
});
